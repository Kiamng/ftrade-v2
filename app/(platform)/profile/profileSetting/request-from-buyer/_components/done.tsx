...
import { doneRequestHistoryColumns } from "./done-columns";
...
const DoneRequestPage = ({ token, userId }: DoneRequestPageProps) => {
  const [doneRequests, setDoneRequests] = useState<doneRequestData[]>([]);
  ...
  useEffect(() => {
    const fetchDoneRequestList = async () => {
      try {
        ...
        setDoneRequests(transformedRequests);
  ...
  return (
    <div className="w-full">
      <DataTable columns={doneRequestHistoryColumns} data={doneRequests} />
    </div>
  );
};
export default DoneRequestPage;
